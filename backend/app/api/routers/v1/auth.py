...
from app.api.deps import SessionDep, CurrentUser, set_access_token_cookie, set_refresh_token_cookie, get_refresh_cookie_requierd
from app.repository.users import UserRepository
...
async def create_user(session: SessionDep, response: Response, user_in: UserRegister) -> Any:
    ...
    return UserWithToken(**user.__dict__, **access_token, **refresh_token)

@router.post("/login", status_code=status.HTTP_200_OK)
async def login_access_token(session: SessionDep, response: Response, form_data: Annotated[OAuth2PasswordRequestForm, Depends()]) -> Token:
    user_repository = UserRepository(session)
